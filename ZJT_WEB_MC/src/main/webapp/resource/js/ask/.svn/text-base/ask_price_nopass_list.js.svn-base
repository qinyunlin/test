...
var rightClick = new Ext.menu.Menu({
			...
			items : [{
						...
						handler : toexamineAskPrice //showaskinfo
					}]
		});
	...
	ds = new Ext.data.SelfStore({
				...
				reader : new Ext.data.JsonReader({
						...
						["id", "corpName", "name", "spec", "memberID","degree",
								"createOn", "updateOn", "updateBy", "revNum","province","city","addr","score","status","notes","auditor","issueDate"]),
				baseParams : {
					type : 4,
					isLock : 0,
					pageSize : 20,
	...
	if (ids.length > 0) {
		Ext.MessageBox.confirm("提示", "您确定删除询价并返还用户询价数吗？", function(op) {
			if (op == "yes") {
				...
				}, "type=27&id=" + ids.toString() + "&memberid=" + memberids.toString());
			}
		});
	} else {
		Ext.MessageBox.alert("提示", "请选择信息。");
...
function toexamineAskPrice(){
	...
	if (ids.length == 1){
		var row = grid.getSelectionModel().getSelected();
		window.parent.createNewWidget("ask_price_nopass_detail", '查看询价',
				'/module/ask/ask_price_nopass_detail.jsp?id=' + row.get("id"));
	}else{
		...
		fn : function(
			...
			if ("ok" == btn) {
				Ext.lib.Ajax
						.request(
								"post",
								"/ask/AskPriceServlet.do?type=39",
								{
									success : function(
											response) {
										var data = eval("("
...
function showaskinfo() {
	...
	if (ids.length == 1){
		...
		window.parent.createNewWidget("ask_info", '普通询价信息',
				'/module/ask/ask_info.jsp?id=' + row.get("id") + "&currStatus=" + row.get("status"));
	}else{
		Ext.MessageBox.alert("提示", "请选择一条信息。");
	}
				...
				success : function(response) {
					...
					if (getState(data.state, commonResultFunc, data.result)) {
						...
						if (!Ext.isEmpty(data.result))
							Ext.getCmp("reply_text")
									.setValue(data.result[length].content);
					}
				},
				failure : function() {
					Warn_Tip();
	...
	Ext.Ajax.request({
				...
				success : function(response) {
					...
					if (getState(data.state, commonResultFunc, data.result)) {
						...
						Ext.getCmp("note_note")
								.setValue(data.result[0].content);
						Ext.getCmp("note_phone").setValue(data.result[0].phone);
						Ext.getCmp("note_linkman")
								.setValue(data.result[0].linkman);
...
function exportaskinfo() {
	...
	window.document.exportform.action = "/AskPriceExportServlet?isLock=0&name=" + name + "&memberID=" + memberID + "&corpName=" + corpName + "&province=" + province +"&city~"+ city + "&isreply=" + isreply;
