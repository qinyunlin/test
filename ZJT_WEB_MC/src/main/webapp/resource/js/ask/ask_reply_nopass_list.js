...
var rightClick = new Ext.menu.Menu({
	...
	items : [ {
		...
		id : 'rMenu3',
		text : '删除',
		hidden : compareAuth("ASK_REP_DEL"),// compareAuth("ASK_VIEW"),
		handler : delAskReply
	} ]
});
		...
		tbar : [ {
			...
			handler : function() {
				...
				if (ids.length == 1) {
					...
					Ext.MessageBox.alert("提示", "请勾选一条回复！");
				}
			}
		},{
			text : '删除',// '打开',
			...
			handler : function() {
				...
				if (ids.length == 1) {
					var rec = grid.getSelectionModel().getSelected();
					delAskReply(rec.data.id,rec.data.pid);
				} else {
					Ext.MessageBox.alert("提示", "请勾选一条回复！");
				}
			}
...
function showaskinfo() {
	...
	if (ids.length == 1) {
		 ...
		 */
		window.parent.createNewWidget("ask_reply_toexamine", '查看审核不通过回复',
...
function delAskReply(){
	...
	if (ids.length == 1) {
		var row = grid.getSelectionModel().getSelected();
		Ext.MessageBox.confirm("提示", "您确定删除该回复吗？", function(op) {
			if (op == "yes") {
				...
				}, "id=" + row.get("id") + "&askId=" + row.get("pid"));
			}
		});
	} else {
		Ext.MessageBox.alert("提示", "请选择一条信息。");
