...
function buildReplyArea() {
	...
	grid = new Ext.grid.GridPanel({
				...
				tbar : [{
							...
							handler : add_win,
							hidden : compareAuth("ASK_REP_ADD")
						}],
				columns : [new Ext.grid.RowNumberer(), {

							header : '回复内容',
							dataIndex : 'name',
							width : parent.Ext.fly('tab_ask_info_iframe')
									.getWidth(),
							sortable : false,
	...
	function renderContent(value, p, record) {
		...
		var str = String
				.format(
						...
						record.data.supplier, record.data.unit,
						...
						record.data.createOn.slice(0, 19),
						record.data.issueDate.slice(0, 19),
						record.data.notes == null ? "" : record.data.notes,
						record.data.area == null ? "" : record.data.area,
						record.data.trueName == null? "": record.data.trueName);
		str = str + '<div class="button_div">';
		if (!compareAuth("ASK_REP_DEL")) {
			str = str
					+ '<input class="button" type="button" value="删除回复" onclick="del_reply();"/>';
		}
		if (!compareAuth("ASK_REP_ADDMAT")) {
			str = str
...
var rightClick = new Ext.menu.Menu({
			...
			items : [{
						...
						text : '添加此材料到材料库',
						handler : addtTomat,
						hidden : compareAuth("ASK_REP_ADDMAT")
					}]
		});
...
function add_win() {
	...
	fs = new Ext.form.FormPanel({
				...
				layout : "table",
				autoWidth : true,
				height : parent.Ext.fly('tab_ask_info_iframe').getHeight() / 2,
				layoutConfig : {
					columns : 2
				},
				labelWidth : 80,
...
function del_reply() {
	...
	if (ids.length > 0) {
		Ext.MessageBox.confirm("提示", "您确定删除该信息吗？", function(op) {
			if (op == "yes") {
				Ext.lib.Ajax.request("post", "/ask/AskServlet?type=14", {
					success : function(response) {
						var data = eval("(" + response.responseText + ")")
...
function add_reply() {
	...
	Ext.lib.Ajax.request("post", "/ask/AskServlet?type=13", {
			...
			}, encodeURI("content=pid~" + selectinfo + ";name~" + name
					+ ";spec~" + spec + ";unit~" + uunit + ";priceGov~"
					+ govprice + ";amount~" + amount + ";priceFac~" + facprice
					+ ";priceAdv~" + advprice + ";supplier~" + supplier
					+ ";brand~" + brand + ";cid~" + cid + ";code~" + code
					+ ";area~" + area + ";issueDate~" + issueDate + ";tax~"
					+ tax + ";carriage~" + carriage + ";keepFee~" + keepFee
					+ ";notes~" + notes + ";askId~" + memid + ";askName~"
					+ memname));
};
