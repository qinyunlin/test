...
var eid,ds, grid, panel,memberid,id,eid;
//商铺类型
...
function init(){
	...
	eid = getCurArgs("eid");
	if (eid == null || "null" == eid || "undefined" == eid){
		eid = "";
	}
	buildForm();
	getShopInfo();
...
function getEffectDate(eid){
	if (eid == null || "" == eid) return;
	Ext.lib.Ajax.request("post", "/ep/EpShopServlet?type=37&eid=" + eid,{
		success : function(response){
			var data = eval("(" + response.responseText + ")");
			if (getState(data.state, commonResultFunc, data.result)){
				if (data.result != null){
					Ext.getCmp('date_i').setValue(data.result.split(" ")[0]);	//有效日期
				}
			}
		},
		failure : function() {
...
function getShopInfo(){
	Ext.lib.Ajax.request("post", "/wcwUser/wcwUserServlet.do?method=getUserInfo",{
		success : function(response){
			var data = eval("(" + response.responseText + ")");
			if (getState(data.state, commonResultFunc, data.result)){
			//Ext.getCmp('date_i').setValue(data.result["validDate"].split(" ")[0]);	//有效日期
			Ext.getCmp('name').setValue(data.result["name"]);	//公司名称
			...
			Ext.getCmp('contact').setText(data["result"].contact);
			Ext.getCmp('sex').setText(data["result"].sex);
			Ext.getCmp('department').setText(data["result"].department);
...
var numReg = /^\d+$/;
//审核诚信商铺信息
function auditChengXinShop(){
	var validDate = Ext.fly("date_i").getValue();
	var name=Ext.fly("name").getValue();
	var regAddress=Ext.fly("regAddress").getValue();
	var regCapital=Ext.fly("regCapital").getValue();
	var regTime=Ext.fly("regTime").getValue();
	var regNum=Ext.fly("regNum").getValue();
	var regOffice=Ext.fly("regOffice").getValue();
	var startTime=Ext.fly("startTime").getValue();
	var endTime=Ext.fly("endTime").getValue();
	var businessScope=Ext.fly("businessScope").getValue();
	var corpn=Ext.fly("corpn").getValue();
    //var logo = FileUpload_Ext.callbackMsg;
	var enterpriseType = Ext.fly("enterpriseType").getValue();
	...
	//query = "memberID=" + memberid + "&validDate=" + validDate + "&catalogId=" + catalogId+"&content="+content+"&integrityLogo="+logo;
    query = "eid=" + eid + "&memberID=" + memberid + "&effectDate=" + validDate + "&catalogId=" + catalogId+"&content="+content;
	
	Ext.MessageBox.confirm("确认操作", "您确定要审核为诚信商铺吗?", function(op) {
		if(op == "yes"){
			Ext.lib.Ajax.request("post","/ep/EpShopServlet?type=26",{
						success : function(response) {
							...
							if (getState(jsondata.state, commonResultFunc,jsondata.result)) {
								// 返回生成的企业ID
								createPic(jsondata.result);
								Ext.MessageBox.alert("提示", "审核成功",closeWin);
								//ds.reload();
								/*var href="/module/enterprise/enterprise_shop_manage.jsp";
								window.parent.createNewWidget("enterprise_shop_manage", '商铺管理',href);*/
								if (window.parent.tab_0309_iframe){
									window.parent.tab_0309_iframe.ds.reload();
...
function closeWin() {
	window.parent.Ext.getCmp('center').remove("audit_chengxin_enterprise_shop");
};
