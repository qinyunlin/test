...
function getEpInfo(eid){
	Ext.lib.Ajax.request("post", "/ep/EpAccountServlet?type=21&eid=" + eid,{
			...
			var data = eval("(" + response.responseText + ")");
				if (getState(data.state, commonResultFunc, data.result)){
					if (data.result != null) {
						...
						var regTime = data.result["regTime"];
						if (regTime != null && "" != regTime){
							Ext.getCmp('regTime').setValue(regTime.split(" ")[0]);	//成立时间
						}
						...
						if (startTime != null && "" != startTime){
							Ext.getCmp('startTime').setValue(startTime.split(" ")[0]);	//营业期限-开始时间
						}
						var endTime = data.result["endTime"];
						if (endTime != null && "" != endTime){
							Ext.getCmp('endTime').setValue(endTime.split(" ")[0]);//营业期限-结束时间
						}
						...
						Ext.MessageBox.alert("提示", eid + "对应的采购企业不存在!");
						return;
					}
			}
		},
			    			 		 ...
			    			 		 border : false,
			   		    			   ...
			   		    			   {xtype:'label',text:'企业ID:'},
			   		    			{
				    						...
				    						listeners: {  
					                            'blur': function(f){  
					                                //获取当前企业信息
					                            	var currEid = Ext.fly("eid").getValue();
					                            	if (currEid != null && "" != currEid){
					                            		getEpInfo(currEid);
					                            	}
					                            }  
				    						} 
...
//审核诚信商铺信息
function createPic(){
	var eid = Ext.fly("eid").getValue(); //企业ID
	var effectDate = Ext.fly("date_i").getValue(); //有效日期
	...
	if(isEmpty(eid) || isEmpty(name) || isEmpty(name) ||isEmpty(regAddress) ||isEmpty(regCapital) ||isEmpty(regTime) ||isEmpty(regNum) 
			||isEmpty(regOffice) ||isEmpty(startTime) ||isEmpty(endTime) ||isEmpty(businessScope)||isEmpty(corpn)){
		this.focus();
		Ext.MessageBox.alert("提示", "请填写完整信息!");
		return;
	}
	if (isEmpty(effectDate)) {
	...
	query = "eid=" + eid + "&flag=add&effectDate=" + effectDate +"&content="+content;
...
function saveInfo(){
	...
	var corpn=Ext.fly("corpn").getValue();//法人代表
	var enterpriseType = Ext.fly("enterpriseType").getValue();//企业类型
	if(isEmpty(eid) || isEmpty(name) ||isEmpty(regAddress) ||isEmpty(regCapital) ||isEmpty(regTime) ||isEmpty(regNum) 
			||isEmpty(regOffice) ||isEmpty(startTime) ||isEmpty(endTime) ||isEmpty(businessScope)||isEmpty(corpn)){
		this.focus();
		Ext.MessageBox.alert("提示", "请填写完整信息!");
		return;
    ...
    var content="regAddress~"+regAddress+";regCapital~"+regCapital+";regTime~"
                 +regTime+";regNum~"+regNum+";regOffice~"+regOffice+";startTime~"+startTime
                 +";endTime~"+endTime+";businessScope~"+businessScope+";corpn~"+corpn+";enterpriseType~"+enterpriseType;
	query = "ename=" + name + "&eid=" + eid + "&flag=add&effectDate=" + effectDate +"&content="+content;
	
	Ext.MessageBox.confirm("确认操作", "您确定要将该采购商添加为诚信采购商吗?", function(op) {
		if(op == "yes"){
			Ext.lib.Ajax.request("post","/ep/EpAccountServlet?type=19",{
				success : function(response) {
					var jsondata = eval("(" + response.responseText + ")");
					if (getState(jsondata.state, commonResultFunc,jsondata.result)) {
						//诚信商铺信息修改成功，则生成对应的认证证书
...
function closeWin() {
	if (window.parent.tab_credit_purchase_ep_list_iframe){
		window.parent.tab_credit_purchase_ep_list_iframe.ds.reload();
	}
	window.parent.Ext.getCmp('center').remove("credit_purchase_ep_add");
};
