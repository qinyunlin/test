...
function buildPanel() {
	...
	grid2 = new Ext.grid.GridPanel({
				...
				columns : [new Ext.grid.RowNumberer({
							...
							dataIndex : 'empStatus',
							renderer : function(value, meta, record) {
								var empStatus = record.get("empStatus");
								//子帐号状态(默认0-服务中，1-服务到期未续费)
								if ("1" == empStatus){
									return "服务到期未续费";
								}
								return "服务中";
...
function saveEmpInfo(mid) {
	Ext.lib.Ajax.request("post", "/ep/EpMemberServlet?type=15", {
				success : function(response) {
					var data = eval("(" + response.responseText + ")");
					if (getState(data.state, commonResultFunc, data.result)) {
						Info_Tip("企业会员添加成功。");
...
function delEmpMem(mid) {
	...
	Ext.lib.Ajax.request("post", "/ep/EpMemberServlet?type=16", {
				success : function(response) {
					var data = eval("(" + response.responseText + ")");
					if (getState(data.state, commonResultFunc, data.result)) {
						Info_Tip("删除成功。");
...
function showPas() {
	...
	var mem_type_ds = new Ext.data.SimpleStore({
				...
				data : [['1', '试用会员'], ['5', '成为赠送会员'], ['7', '成为内部会员'],
						['3', '成为正式信息会员'], ['8', '成为VIP会员'], ['9', '成为企业会员'],
						['2', '试用厂商会员'], ['4', '正式厂商会员']]
			});
	win = new Ext.Window({
		title : "审核",
...
function stopService() {
	...
	var empStatus = row.get("empStatus");
	if ("1" == empStatus){
		Info_Tip("该子帐号已设置为服务到期，无需重复设置！");
		return;
	}
	sel_mid = row.get("memberID");
	Ext.lib.Ajax.request("post", "/ep/EpMemberServlet?type=21", {
				success : function(response) {
					var data = eval("(" + response.responseText + ")");
					if (getState(data.state, commonResultFunc, data.result)) {
						Info_Tip("设置成功。");
...
function activeService(){
	...
	var empStatus = row.get("empStatus");
	if ("0" == empStatus){
		Info_Tip("该子帐号已激活服务，无需重复设置！");
		return;
	}
	sel_mid = row.get("memberID");
	Ext.lib.Ajax.request("post", "/ep/EpMemberServlet?type=22", {
				success : function(response) {
					var data = eval("(" + response.responseText + ")");
					if (getState(data.state, commonResultFunc, data.result)) {
						Info_Tip("设置成功。");
