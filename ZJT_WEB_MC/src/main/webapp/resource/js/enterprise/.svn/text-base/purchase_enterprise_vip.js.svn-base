...
var eidGlobal;
var isShows = [];var ppgysProvince="";
var brandFacProvinces = [];
function init() {
	Ext.QuickTips.init();
	buildGrid();
};

function vipEpAccount(thiseid){
	Ext.lib.Ajax.request("post", "/ep/EpAccountServlet?type=11", {
		success : function(response) {
			var jsondata = eval("(" + response.responseText + ")");
			if (getState(jsondata.state, commonResultFunc,
					jsondata.result)) {
				ppgysProvince=jsondata.result["brandFacProvince"];
...
var toolbar = [{
			text : 'VIP权限设置',
			hidden : compareAuth('VIP_ADMIN_RIGHT'),
			handler : getEmpAuth,
			hidden : compareAuth('VIP_ADMIN_RIGHT'),
			...
			hidden : compareAuth('VIP_ADMIN_ADD'),
			handler : openVipLib
		}, {
			text : '导入供应商模板',
			hidden : compareAuth('VIP_TEMP_ADD'),
			handler : importArea,
			icon : '/resource/images/table_row_insert.png'
		}, {
			text : '锁定',
			hidden : compareAuth('VIP_ADMIN_LOCK'),
			cls : 'x-btn-text-icon',
			...
			text : '查看已锁定采购企业',
			hidden : compareAuth('PURCHASE_EP_LOCK_LIST'),
	...
	ds = new Ext.data.SelfStore({
				...
				reader : new Ext.data.JsonReader({
							root : 'result'
								...
								"mod_Sys", "mod_Users", "validDate","isIntegrity","effectDate"]),
				baseParams : {
					page : 1,
					type : 5,
					pageSize : pageSize
...
function openVipLib() {
	...
	var vip_module = new Ext.grid.GridPanel({
				...
				columns : [new Ext.grid.RowNumberer({
							...
							sortable : false,
							dataIndex : 'name',
							renderer: function (data, metadata, record, rowIndex, columnIndex, ds2) {  
							      var  total= ds2.getAt(rowIndex).get('name');  
							      if(total=="品牌供应商库"){
							    	  return "<a href='javascript:void(0);' onclick='viewBrandFacArea();'>查看/选择区域</a>";
							      }
							      //return '<a onclick="linkShop('+"'"+province+"'"+','+null+','+"'"+ds2.getAt(rowIndex).get('city')+"'"+')" href="#">' +total+'</a>';  
...
function searchlist() {
	...
	var temp = query_con + "~" + query_input + ";degree~" + query_type;
...
function getEmpAuth() {
	...
	thiseid = row.get("eid");
	vipEpAccount(thiseid);
	Ext.lib.Ajax.request("post", "/ep/EpAccountServlet?type=23", {
				success : function(response) {
					var jsondata = eval("(" + response.responseText + ")");
					if (getState(jsondata.state, commonResultFunc,
							jsondata.result)) {
						
						showVipModule(thiseid, jsondata.result,ppgysProvince);
					}
				},
				failure : function() {
					Warn_Tip();
								...
								handler : function(){
									...
									Ext.select("input[name=webProvince_checkbox]").each(function(el) {
												if (Ext.getDom(el).checked){
													brandFacProvinces.push(el.getValue());
												}
											});
									win_gov.close();
								}
...
function province_select(){
	...
	var len = pro.length;
	for (var i = 0; i < len; i++) {
		if(ppgysProvince!=null && ppgysProvince!=""){
			if(ppgysProvince.indexOf(pro[i])!=-1){
				tempObj.push({
					columnWidth:.5,
	...
	Ext.Msg.confirm("提示", "您确定要保存此设置?", function(op) {
				if (op == "yes") {
					Ext.lib.Ajax.request("post",
							...
							}, "eid=" + thisid + "&codes=" + mids.toString()+"&brandFacProvince="+brandFacProvinces.toString());
				}
			});
};
// 设定VIP模块权限
