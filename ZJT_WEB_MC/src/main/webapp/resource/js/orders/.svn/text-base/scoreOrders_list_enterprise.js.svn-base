...
// 查询条件
var searchDiyArr = [ [ 'tVipScoreOrders.code', '订单编号' ],
		[ 'tVipScoreOrders.memberId', '会员帐号' ], [ 'tEnterprise.name', '公司名称' ] ];
...
function buildGirid() {
	...
	ds = new Ext.data.SelfStore({
		...
		reader : new Ext.data.JsonReader({
			root : 'result',
			id : 'id'
		}, [ "id", "code", "eid", "memberId", "score", "money", "type", "orderStatus", "status",
				"createOn", "createBy", "updateOn", "updateBy", "paymentOn", "name", "invoice", "sendInvoice","isAct","corpName" ]),
		baseParams : {
			type : 21,
			pageSize : pageSize,
			pageNo : 1
...
function searchlist() {
	...
	var content = "tVipScoreOrders.orderStatus~" + orderStatusVal + ";tVipScoreOrders.type~" + orderTypeVal + ";"
			+ searchDiyName + "~" + searchDiyVal + ";tVipScoreOrders.status~" + orderStatus;
...
function showOrders() {
	...
	window.parent.createNewWidget("scoreOrders_detail_enterprise", '查看详细',
			'/module/orders/scoreOrders_detail_enterprise.jsp?id=' + id + "&code=" + code);
}
...
function uploadOrderFile() {
	if (upload_form.getForm().isValid()) {
		...
		upload_form.getForm().submit({
				    url : '/mc/order/ScoreOrderServlet.do?type=19&flag=enterprise',
					method:"post",
					//waitMsg : '上传文件中...',
...
function submitForm(append) {
	  var url = "/mc/order/ScoreOrderServlet.do?type=20";
	  if(typeof(append) != "undefined" && append == "enterprise")
		  url = "/mc/order/ScoreOrderServlet.do?type=26";
