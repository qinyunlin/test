...
// 查询条件
var searchDiyArr = [ [ 'vipspace.tVipRechargeOrders.code', '订单编号' ],
		[ 'vipspace.tVipRechargeOrders.eid', '企业ID' ], [ 'tMember.corpName', '公司名称' ] ];
...
function buildGirid() {
	...
	ds = new Ext.data.SelfStore({
		...
		reader : new Ext.data.JsonReader({
			...
			id : 'id'
				...
				"createOn", "createBy", "paymentOn", "trueName", "corpName","eid" ]),
		baseParams : {
			type : 16,
			pageSize : pageSize,
			pageNo : 1
	...
	grid_info = new Ext.grid.EditorGridPanel({
		...
		columns : [ new Ext.grid.RowNumberer({
			...
			renderer : function(value, p, record) {
				...
				var status = record.get("status");
				if ("1" == status){
					return "<font color='red'>" + eid + "</font>";
				}
				return eid;
			}
		}, {
	...
	var tbar1 = new Ext.Toolbar({
		...
		items : [ {
			...
			handler : moneyHandle,
			hidden : compareAuth("VIP_EP_RECHARGE_RECONCILIATION")
		}, "-", {
			text : '账户明细',
			icon : "/resource/images/book_edit.png",
			handler : showAccountDetail,
			hidden : compareAuth("VIP_EP_RECHARGE_ACCOUNTDETAIL")
		} ]
	});
	...
	var bar2 = new Ext.Toolbar({
		...
		items : [ {
			...
			displayField : "text",
			width : 100,
			value : 'vipspace.tVipRechargeOrders.code'
		}, "-", {
			xtype : "textfield",
			textLabel : "查询条件",
			id : "searchDiyVal",
...
function searchlist() {
	...
	var content = "vipspace.tVipRechargeOrders.orderStatus~" + orderStatusVal + ";"
			+ searchDiyName + "~" + searchDiyVal + ";vipspace.tVipRechargeOrders.status~" + orderStatus;
...
function showAccountDetail() {
	window.parent.createNewWidget("vipAccountDetail_list", '账户详细',
			'/module/orders/vipAccountDetail_list.jsp');
}
