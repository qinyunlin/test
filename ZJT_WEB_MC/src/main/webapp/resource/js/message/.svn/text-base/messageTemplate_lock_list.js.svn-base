...
function buildGirid() {
	...
	grid_info = new Ext.grid.GridPanel({
		...
		tbar : [ {
			...
			hidden : compareAuth("MESSAGETEMPLATE_UNLOCK"),
			handler : unLockMessageTemplate
		}, "-", {
			text : '删除',
			cls : 'x-btn-text-icon',
			icon : '/resource/images/delete.gif',
			hidden : compareAuth("MESSAGETEMPLATE_DEL"),
			handler : deleteMessageTemplateList
		}],
		bbar : pagetool,
...
function viewMessageTemplate(){
	...
	window.parent.createNewWidget("messageTemplate_edit", '查看/修改模板',
			'/module/message/messageTemplate_edit.jsp?id=' + row.get("id"));
}

/**
 * 解锁模板
 */
function unLockMessageTemplate(){
	var row = grid_info.getSelectionModel().getSelected();
	if (Ext.isEmpty(row)) {
		Info_Tip("请选择一条模板。");
		return;
	...
	.request({
		...
		success : function(response) {
			...
			if (getState(json.state,
					commonResultFunc,
					json.result)) {
				Ext.MessageBox.alert("提示", "成功解锁模板！");
				ds_info.load();
			} 
		},
		failure : function() {
 ...
 * 删除模板
 */
function deleteMessageTemplateList(){
	var row = grid_info.getSelectionModel().getSelected();
	if (Ext.isEmpty(row)) {
		Info_Tip("请选择一条模板。");
		return;
	...
	.request({
		...
		success : function(response) {
			...
			if (getState(json.state,
					commonResultFunc,
					json.result)) {
				Ext.MessageBox.alert("提示", "成功删除模板！");
				ds_info.load();
			} 
		},
		failure : function() {
