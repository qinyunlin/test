...
var memberTypeArr = [['10,13',"全部采购会员"],[10,"正式采购会员"],[13,"试用采购会员"]];
...
var tbar = [{
			...
			hidden : compareAuth("PURCHASE_MEM_LOCK"),
			handler : lockfun
		}, {
			text : '审核',
			icon : "/resource/images/tick.png",
			hidden : compareAuth("PURCHASE_MEM_AUDIT"),
			handler : showPas
		}, {
			text : '续期',
			icon : "/resource/images/table_multiple.png",
			hidden : compareAuth("PURCHASE_MEM_RENEW"),
			handler : showKeep
		}/*,{
			text : '关联企业',
			icon : "/resource/images/house_link.png",
			hidden : compareAuth("PURCHASE_MEM_RELEVANCE"),
			...
			hidden : compareAuth("PURCHASE_MEM_EXPORT"),
			handler : exportExcel
		}, {
			text : '已锁定采购会员',
			icon : "/resource/images/edit.gif",
			handler : openLockMemberList,
			hidden : compareAuth('PURCHASE_MEM_LOCK_LIST')
		}];
...
function exportExcel() {
	...
	if (Ext.isEmpty(ds_info.baseParams["diff"])) {
		window.document.exportform.action = "/mc/Member.do?method=export&exportMemType=10&ids="
...
function openInfo() {
	...
	window.parent.createNewWidget("purchase_member_info", '采购会员信息',
			'/module/member/purchase_member_info.jsp?id=' + id);
};
...
function showKeep() {
	...
	var rows = grid_info.getSelectionModel().getSelections();
	for (var i = 0; i < rows.length; i++) {
		if ("10" != rows[i].get("degree")){
			Ext.Msg.alert("提示", "此操作只能对正式采购会员续期，请选择正式采购会员！");
			return;
		}
	}
...
function showPas() {
	...
	var row = grid_info.getSelectionModel().getSelected();
	if ("13" != row.get("degree")){
		Ext.Msg.alert("提示", "此操作只能审核试用采购会员，请重新选择一条试用采购会员信息！");
		return;
	}
	
	var eid = row.get("EID");
	if (eid == null || "" == eid){
		Ext.Msg.alert("提示", "该试用采购会员必须先关联企业才可进行审核！");
		return;
	}
...
function changeGovArea() {
	...
	win_gov = new Ext.Window({
				...
				items : [{
					...
					items : [{
								...
								listeners : {
									select : function(combo, record, index) {
										var province = combo.getValue();
										if(province == "全部省份") {
											Ext.getCmp('info_province').setText("全国");
											city = ["全部城市"];
										}else{
											city = zhcn.getCity(province).concat();
											city.unshift("全部城市");
											Ext.getCmp('info_province').setText(province);
										}
										/*Ext.getCmp('info_province').setText(province);*/
										Ext.getCmp('info_city').setText('');
										Ext.getCmp('city_one').store.loadData(city);
										Ext.getCmp('city_one').setValue("全部城市");
										Ext.getCmp('city_one').enable();
									}
								}
							}, 
							{
							...
							listeners : {
								select : function(combo, record, index) {
									var city = combo.getValue();
									if(city == "全部城市") {
										Ext.getCmp('info_city').setText(' ');
									}else{
										Ext.getCmp('info_city').setText(' '+city);
									}
					...
					layout : 'form',
					bodyStyle : 'border:none;background-color:#fff;margin-top:20px;',
					items:[{
						xtype : 'label',
						id : 'text',
						text:'当前账号访问区域权限为:',
...
function showEmpAction() {
	...
	emp_grid = new Ext.grid.GridPanel({
				store : emp_ds,
				autoWidth : true,
				height : parent.Ext.fly('tab_0203_iframe').getHeight() / 2,
				autoScroll : true,
				stripeRows : true,
				loadMask : true,
				columns : [new Ext.grid.RowNumberer({
...
function openLockMemberList(){
	window.parent.createNewWidget("purchase_member_lock_list", '已锁定采购会员',
	'/module/member/purchase_member_lock_list.jsp');
}