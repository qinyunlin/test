...
function EpShopCountByCid(province){
...
storeCity = new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
						root : 'result'
					}, ["total", "ordinary", "midRange", "midGrade", "highGrade","cname","cid"]),
			baseParams : {
				type : 4,
 ...
 var grid3 = new Ext.grid.GridPanel({
			...
			columns : [{
				...
				renderer: function (data, metadata, record, rowIndex, columnIndex, storeCity) {  
				      ...
				      var cid= storeCity.getAt(rowIndex).get('cid');
...
function EpShopCountByProvince() {
	...
	expander.expandRow = function(row){
		 if(typeof row == 'number'){
        	...
        	rowTemp = row;
        	EpShopCountByCid(recordTemp.data.province);
