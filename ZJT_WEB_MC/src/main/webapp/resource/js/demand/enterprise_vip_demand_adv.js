Ext.onReady(init);
var grid, ds, win, pageSize = 20, state_ck, fs, stuff_type, zaojia_type, his_grid, ds2;
// 供应一级分类store
...
var rightClick = new Ext.menu.Menu({
			...
			items : [{
						...
						text : '解锁',
						// hidden : compareAuth('VIP_DEMAND_LOCK'),
						hidden : compareAuth('DEMAND_ADMIN_UNLOCK'),
						handler : unlockop
					}, {
						id : 'rMenu4',
						text : '删除',
						// hidden : compareAuth('VIP_DEMAND_DEL'),
						hidden : compareAuth('DEMAND_ADMIN_DEL'),
						handler : delInfo
					}]
		});
function buildGrid() {
	ds = new Ext.data.SelfStore({
				...
				reader : new Ext.data.JsonReader({
							root : 'result'
								...
								"addr", "startTime", "endTime", "isAudit",
								"updateBy", "isLock", "attCount", 'createBy',
								'cname', 'cname1']),
				baseParams : {
					page : 1,
					type : 14,
	...
	var bar = new Ext.Toolbar({
				renderTo : grid.tbar,
				items : [searchPanel/*
									 * { layout : 'form', labelWidth : 50, xtype :
									 * 'container', items : { xtype :
									 * 'textfield', fieldLabel : '项目名称', id :
									 * 'pro_name', enableKeyEvents : true,
									 * listeners : { "keyup" : function(tf, e) {
									 * if (e.getKey() == e.ENTER) {
									 * searchlist(); } } } } }, "-", { layout :
									 * 'form', labelWidth : 50, xtype :
									 * 'container', items : { xtype :
									 * 'textfield', fieldLabel : '采购名称', id :
									 * 'demand_name', enableKeyEvents : true,
									 * listeners : { "keyup" : function(tf, e) {
									 * if (e.getKey() == e.ENTER) {
									 * searchlist(); } } } } }, "-", { layout :
									 * 'form', labelWidth : 50, xtype :
									 * 'container', items : { xtype :
									 * 'textfield', fieldLabel : '企业名称', id :
									 * 'demand_ename', enableKeyEvents : true,
									 * listeners : { "keyup" : function(tf, e) {
									 * if (e.getKey() == e.ENTER) {
									 * searchlist(); } } } } }, "-", { layout :
									 * 'form', labelWidth : 50, xtype :
									 * 'container', items : [state_ck = new
									 * Ext.form.ComboBox({ mode : "local", store :
									 * del_store, triggerAction : "all",
									 * valueField : 'value', displayField :
									 * 'text', fieldLabel : '审核状态', value : "",
									 * width : 80, listeners : { "select" :
									 * function() { searchlist(); } } })] },
									 * "-", { text : '查询', icon :
									 * "/resource/images/zoom.png", handler :
									 * searchlist }
									 */]
			});
...
function delInfo() {
	...
	Ext.MessageBox.confirm("确认操作", "您确认要删除选中的信息吗？", function(op) {
				if (op == "yes") {
					Ext.lib.Ajax.request("post", "/ep/EpDemandServlet?type=16",
							{
...
function addDemand(obj_info, op) {
	...
	if (op == "add") {
		...
		fs = new Ext.form.FormPanel({
			layout : 'form',
			labelWidth : 60,
			height : parent.Ext.fly('tab_1204_iframe').getHeight() / 2,
			autoScroll : true,
			autoWidth : true,
	...
	win = new Ext.Window({
				...
				buttons : [{
					text : btn_text,
					hidden : op == "edit"
							? compareAuth('DEMAND_ADMIN_MOD')
							: compareAuth('DEMAND_ADMIN_VIEW'),
					handler : function() {
						saveDemand(op);
					}
				}, {
	...
	if (obj_info) {
		...
		Ext.fly("d_starttime").dom.value = obj_info["startTime"].slice(0, 10);
		Ext.fly("d_endtime").dom.value = obj_info["endTime"].slice(0, 10);
	}
};
...
function saveDemand(op) {
	...
	if (fs.getForm().isValid()) {
		if (op == "add" || op == "edit") {
			var ispass = Date.parse(Ext.fly("d_endtime").getValue().replace(
					/-/g, "/"))
					- Date.parse(Ext.fly("d_starttime").getValue().replace(
							/-/g, "/"))
			if (ispass < 0) {
			...
			query += "title~" + fs.getForm().items.map["d_title"].getValue()
					...
					+ ";content~"
					+ fs.getForm().items.map["d_content"].getValue()
					+ ";linkman~"
					+ fs.getForm().items.map["d_person"].getValue() + ";phone~"
					+ fs.getForm().items.map["d_phone"].getValue() + ";addr~"
					+ fs.getForm().items.map["d_addr"].getValue()
					...
					+ fs.getForm().items.map["d_fax"].getValue() + ";photo~"
					+ photo;
		}
		if (op == "add") {
			params = "type=1";
			...
			params = "type=3&id="
					+ grid.getSelectionModel().getSelected().get("id");
			tip = "修改求购信息成功。"
			...
			params = "type=17&id=" + ids.toString() + "&ctype=" + ctype
					+ "&cname=" + cname;
			tips = "修改信息成功!";
		}
		if (op == "add") {
...
function getInfo(op) {
	if (op == "add") {
		...
		Ext.lib.Ajax.request("post", "/ep/EpDemandServlet?type=6", {
					success : function(response) {
						var json = eval("(" + response.responseText + ")");
...
function unlockop() {
	...
	Ext.lib.Ajax.request("post", "/ep/EpDemandServlet?type=7", {
				success : function(response) {
					var json = eval("(" + response.responseText + ")");
					if (getState(json.state, commonResultFunc, json.result)) {
						Info_Tip("操作成功。");
