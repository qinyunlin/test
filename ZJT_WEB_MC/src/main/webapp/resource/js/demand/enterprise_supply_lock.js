...
var rightClick = new Ext.menu.Menu({
			...
			items : [{
						...
						id : 'rMenu2',
						text : '锁定',
						hidden:compareAuth('SUPPLY_ADMIN_UNLOCK'),
						handler : unlockop
					}, {
						id : 'rMenu4',
						text : '删除',
						hidden : compareAuth('SUPPLY_ADMIN_DEL'),
						handler : del
					}]
		});
function buildGrid(){
	ds = new Ext.data.SelfStore({
		...
		reader : new Ext.data.JsonReader({
			root : 'result'
		}, ['id','eid','ename','cname', 'cname1', 'type', 'name','spec','unit','code','brand','price','title','addr','issueDate',
			'validDate','photo','saleArea','attCount','isAudit','isLock','updateOn','createBy','updateBy','createBy',
			'title']),
		baseParams : {
			type : 16,
			isLock:1
...
function searchlist(){
	...
	var content = "ename~" + ename + ";name~" + name + ";saleArea~" + saleArea + ";cname~" + cname + ";cname1~" + cname1 + ";type~" + type;
...
function unlockop() {
	...
	Ext.lib.Ajax.request("post", "/ep/EpSupplyServlet?type=13", {
				success : function(response) {
					var json = eval("(" + response.responseText + ")");
...
function buildSupplyInfo(op, data){
	...
	if(op == "add"){
		...
		fs = new Ext.form.FormPanel({
			layout : 'form',
			labelWidth : 60,
			height : parent.Ext.fly('tab_1203_iframe').getHeight()/2,
			autoScroll:true,
			autoWidth : true,
	...
	win = new Ext.Window({
				...
				buttons : [{
							text : btn_text,
							hidden: op == 'add' ? compareAuth('SUPPLY_ADMIN_VIEW') : compareAuth('SUPPLY_ADMIN_MOD'),
							handler : function() {
								saveSupply(op);
							}
						}, {
...
function saveSupply(op){
	...
	if(op == "add"){
		...
		notes = notes != "" ? notes : " ";
		var content = "title~" + title + ";name~" + name + ";cname~" + cname + ";cname1~" + cname1 + ";spec~" + spec 
			+ ";unit~" + unit + ";addr~" + addr + ";brand~" + brand + ";validDate~" + validDate
			+ ";type~" + type + ";linkman~" + linkman + ";phone~" + phone + ";qq~" 
			+ qq + ";msn~" + msn + ";email~" + email + ";notes~" + notes;
		var row = grid.getSelectionModel().getSelected();
		...
		data = "type=" + 18 + "&id=" + ids.toString() + "&ctype=" + ctype + "&cname=" + cname;
		tips = "修改信息成功!";
	}
...
function del(){
	...
	Ext.lib.Ajax.request("post", "/ep/EpSupplyServlet?type=17", {
				success : function(response) {
					var json = eval("(" + response.responseText + ")");
					if (getState(json.state, commonResultFunc, json.result)) {
						Info_Tip("删除成功!");
