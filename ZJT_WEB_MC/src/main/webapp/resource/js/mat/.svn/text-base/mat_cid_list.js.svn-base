	...
	items : [{
		...
		handler : function() {
			...
			if (grid.getSelectionModel().getCount() > 0) {
				...
				/*
				 * { var row = grid.getSelectionModel().getSelected(); if
				 * (Ext.isEmpty(row)) { Info_Tip("请选择一条信息。"); return; }
				 * showEditStuff('', row) }
				 */
			} else {
				showEditStuff('all');
...
function buildGrid() {
	ds = new Ext.data.SelfStore({
				...
				reader : new Ext.data.JsonReader({
							root : 'result'
								...
								"p1", "p2", "p3", "p4", "grade", "cid", "addr",
								"brand", "topPhoto", "pricem", "code",
								"code2010", "cname", "cname1", "cname2"]),
				baseParams : {
					type : 'fac_ex',
					// type : 'fac',
					page : 1,
								...
								listeners : {
									'select' : function() {
										...
										if (cname_1 == "所有") {
											...
											cname_2_ds
													.loadData(getAllStuff_cid_2(cname_1));
										}
										cname_2.setValue("");
									}
								}
	...
	var bar = new Ext.Toolbar({
				renderTo : grid.tbar,
				items : [searchPanel/*
									 * { text : "建材一级分类：", xtype : 'label' },{
									 * xtype : 'combo', id : 'cname_1', mode :
									 * 'local', triggerAction : 'all', readOnly :
									 * true, store : cname_1_ds, valueField :
									 * 'value', displayField : 'text', value :
									 * '', listeners : { 'select' : function(){
									 * var cname_1 =
									 * Ext.fly('cname_1').getValue(); var
									 * cname_2 = Ext.getCmp('cname_2');
									 * if(cname_1 == "所有"){
									 * cname_2_ds.removeAll();
									 * cname_2.setDisabled(true); } else{
									 * cname_2.setDisabled(false);
									 * cname_2_ds.loadData(getStuff_cid_2(cname_1)); }
									 * cname_2.setValue(""); } } },{ text :
									 * "建材二级分类：", xtype : 'label' },{ xtype :
									 * 'combo', id : 'cname_2', mode : 'local',
									 * triggerAction : 'all', readOnly : true,
									 * store : cname_2_ds, valueField : 'value',
									 * displayField : 'text', disabled : true,
									 * emptyText : '请选择' }, "-", { text :
									 * "非标分类：", xtype : 'label' }, { xtype :
									 * 'combo', id : 'cname1', mode : 'local',
									 * triggerAction : 'all', readOnly : true,
									 * store : cname2_ds, valueField : 'value',
									 * displayField : 'text', disabled : true,
									 * value : '1' }, "-", { text : "供求一级分类：",
									 * xtype : 'label' }, { xtype : 'combo', id :
									 * 'cname2_1', mode : 'local', triggerAction :
									 * 'all', readOnly : true, store :
									 * cname2_1_ds, valueField : 'value',
									 * displayField : 'text', disabled : true,
									 * value : '' }, { text : "供求二级分类：", xtype :
									 * 'label' }, { xtype : 'combo', id :
									 * 'cname2_2', mode : 'local', triggerAction :
									 * 'all', readOnly : true, store :
									 * cname2_2_ds, valueField : 'value',
									 * displayField : 'text', disabled : true,
									 * value : '' }, "-", { text : "名称：", xtype :
									 * 'label', width: 100 }, { xtype :
									 * 'textfield', id : 'input_name', width:
									 * 100 }, "-", { xtype : 'label', text :
									 * '型号规格：' }, { xtype : 'textfield', id :
									 * 'input_spec', width: 100 }, "-", { text :
									 * '材料产地：', xtype : 'label' }, { xtype :
									 * 'textfield', id : 'input_addr', width:
									 * 100 }, "-", { xtype : "label", text :
									 * '品牌：' }, { xtype : 'textfield', id :
									 * 'input_brand', width: 100 }, { text :
									 * '查询', icon : '/resource/images/zoom.png',
									 * handler : searchlist }
									 */]
			});
	ds.load();
	...
	// saveInfo_list(record.get('id'), e.field, record.data);
	//
	// }, ds);
	...
	grid.on("afteredit", function(e) {
				...
				// debugger;
				saveInfo_list(e.record.id, e.field,
						(e.record.data[e.field] == "未分类"
								? ""
...
function searchlist() {
	...
	var content = "fid~" + eid + ";cname~" + cname + ";cname1~" + cname1
			+ ";cname2~" + cname2;
...
function showEditStuff(v, data) {
	...
	if (Ext.isEmpty(v)) {
		...
		stuff_form = new Ext.form.FormPanel({
					...
					labelWidth : 80,
					items : [ /*
								 * { id : 'cname', xtype : 'combo', fieldLabel :
								 * '建材分类', store : cname_ds, emptyText : "请选择",
								 * mode : "local", triggerAction : "all",
								 * valueField : 'value', displayField : 'text',
								 * readOnly : true, value : data.get("cname") },
								 */{
								id : 'cname1',
								xtype : 'combo',
								fieldLabel : '专题分类',
	...
	if (stuff_form.getForm().isValid()) {
		...
		for (var i = 0; i < len; i++) {
			...
			if (stuff_form.getForm().items.keys[i] == "issueDate")
				ids
						.push(stuff_form.getForm().items.keys[i]
								+ "~"
...
// 进入厂商报价未审核列表
function mat_price() {
	window.parent.createNewWidget("mat_cid_list_u", '修改未审核材料分类',
			'/module/mat/mat_cid_list_u.jsp?eid=' + eid + "&fname="
					+ encodeURI(fname));
};
