...
function showDetail(province,city,town){
storeCity = new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
						root : 'result'
					}, ["id", "province", "city", "town", "year","month","quarter","status","createOn","createBy"]),
			baseParams : {
				type : 13,
				content : "province~" + province + ";city~" + city + ";town~" + town
			},
			remoteSort : false
		});
			...
			columns : [{
				...
				dataIndex : ''
					...
					renderer: function (data, metadata, record, rowIndex, columnIndex, storeCity) {  
						...
						dataIndex : "quarter",
						renderer: function (data, metadata, record, rowIndex, columnIndex, storeCity) {  
							      var  quarter = storeCity.getAt(rowIndex).get('quarter');  
							      if ("0" == quarter){
							    	  return "";
							      }
							      return quarter;  
...
function showAllDetail() {
	var storeProvince = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
							root : 'result',
							fields:[{name:"id",type:'int'},{name:"province"},{name:"city"},{name:"town"},{name:"year",type:'int'},{name:"month",type : "int"},{name:"quarter",type:"int"},{name:"status",type:"int"},{name:"createOn"},{name:"createBy"}]
						}),
				baseParams : {
					type : 13,
					totalFlag : 1
				...
				columns : [expander,{
					...
					renderer:function(data,metadata,record,rowIndex,columnIndex,store){
						metadata.css='x-grid-back-color';
						 ...
						 var  town = store.getAt(rowIndex).get('town');  
						 return province + "-" + city + "-" + town;
							...
							renderer: function (data, metadata, record, rowIndex, columnIndex, store) {  
								metadata.css='x-grid-back-color';
							      var  quarter = store.getAt(rowIndex).get('quarter');  
							      if ("0" == quarter){
							    	  return "";
							      }
							      return quarter;  
