...
var buildGrid = function() {
	...
	ds = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
							root : 'result'
						}, ['id', 'pid', 'path', 'name','CONTENT_TEMPLATE','CONTENT_TEMPLATE1']),
				baseParams : {
					type : 2,
					ctype : ctype,
					id : 0
...
// 增加分类
var create = function(name,id,listId) {
	if(!name){
	...
	if (node) {
		Ext.lib.Ajax.request('post', '/proj/ProjectCatalogServlet', {
				...
				}, "node=" + node.id +"&templateId="+id+"&name=" + name + "&type=5&ctype="
						+ ctype+"&listTemplateId="+listId);
	}
...
var buildAddType = function() {
	...
	addTypeWin = new Ext.Window({
				...
				buttons : [{
							text : '确定',
							handler : function() {
								create(addTypeForm.getForm().getEl().dom.name.value,addTypeForm.getForm().getEl().dom.content_template.value
										,addTypeForm.getForm().getEl().dom.list_template.value);
							}
						}, {
							text : '取消',
							handler : function() {
