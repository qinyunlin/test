...
function buildGrid() {
	ds = new Ext.data.SelfStore({
				...
				reader : new Ext.data.JsonReader({
							...
							id : ["id", "catalog"]
								...
								"emailAddress", "id", "name", "password",
								"server", "updateBy", "updateOn", "username"]),
				baseParams : {
					type : 5,
					cid : 1,
					content : 'isLock~0'
	...
	grid = new Ext.grid.GridPanel({
				...
				tbar : [{
							...
							handler : showAddArea,
							hidden : compareAuth('EMAIL_ACCOUNT_ADD')
						}, {
							text : '修改帐户',
							...
							handler : delAccount,
							hidden : compareAuth('EMAIL_ACCOUNT_LOCK')
						}],
				renderTo : 'grid',
				bbar : pagetool,
				listeners : {
...
function searchlist() {
	content = "name~" + Ext.fly("input_name").getValue() + ";username~"
			...
			+ Ext.fly("input_displayname").getValue() + ";isLock~0";
...
function showAddArea(op) {
	...
	fs = new Ext.FormPanel({
				autoWidth : true,
				height : parent.Ext.fly('tab_1103_iframe').getHeight() / 2,
				layout : 'form',
...
// 进入已锁定列表
function jumpTolock() {
	parent.createNewWidget("email_account_lock", "已锁定帐号管理",
			"/module/email/email_account_lock.jsp");
};