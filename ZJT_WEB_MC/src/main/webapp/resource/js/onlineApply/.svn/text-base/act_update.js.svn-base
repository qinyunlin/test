...
var act_submit = function(){
	if(act_add_form.form.isValid()){
		...
		var actId = Ext.fly("actId").getValue();
		...
		var data = {};
		data["content"] = "actId~" + actId + ";actName~" + actName + ";contactPhone~" + contactPhone + ";bussinessPhone~" + bussinessPhone +
...
var initData = function(){
	...
	Ext.Ajax.request({
		...
		success : function(response){
			var jsondata = eval("(" + response.responseText + ")");
			if (getState(jsondata.state, commonResultFunc, jsondata.result)) {
				var actInfo = jsondata.result;
				act_add_form.getForm().setValues(actInfo);
				Ext.get("picPath").dom.src = FileUpload_Ext.requestURL+ actInfo.picPath;
				Ext.get("actTime").dom.value = trimDate(actInfo.actTime);
			}
		},
		failure : function(){
			Warn_Tip();
